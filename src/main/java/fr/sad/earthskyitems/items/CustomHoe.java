...
public class CustomHoe extends CustomItem {

    public CustomHoe() {
        super("hoe1", new ItemCreator(Material.IRON_HOE).setUnbreakable(true).setName("§6Houe (1)").setLores("", "§7Cette houe permet de casser", "§7et de replanter en 3x3").getItem());
    }
                ...
                if(PermissionUtils.canBuild(event.getPlayer(), event.getClickedBlock().getLocation(), IslandPrivilege.getByName("BREAK"))){
                    ...
                    if(event.getAction() == Action.LEFT_CLICK_BLOCK){
                        for(int x = block.getX() - 1; x <= block.getX() + 1; x++){
                            for(int z = block.getZ() - 1; z <= block.getZ() + 1; z++){
                                Block target = block.getWorld().getBlockAt(x, block.getY(), z);
                                if(PermissionUtils.canBuild(event.getPlayer(), target.getLocation(), IslandPrivilege.getByName("BREAK")) && PermissionUtils.canBuild(event.getPlayer(), target.getLocation(), IslandPrivilege.getByName("BREAK"))){
